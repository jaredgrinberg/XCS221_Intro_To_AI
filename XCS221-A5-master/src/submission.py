...
def create_chain_csp(n):
    ...
    for i in range(1, n):
        ...
        csp.add_binary_factor(variables[i - 1], variables[i], factor_func)
    # ### END CODE HERE ###
    return csp
...
def create_nqueens_csp(n = 8):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            csp.add_binary_factor(variables[i], variables[j], lambda a,b: a!= b and j - i != abs(a - b))
    # ### END CODE HERE ###
    return csp
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            pass
            # ### START CODE HERE ###
            decreasing_number_of_domain = float('inf')
            for variable in self.csp.variables:
                if variable not in assignment:
                    domain_number = 0
                    for var in self.domains[variable]:
                        if self.get_delta_weight(assignment, variable, var) > 0:
                            domain_number += 1
                    if decreasing_number_of_domain > domain_number:
                        decreasing_number_of_domain = domain_number
    ...
    def arc_consistency_check(self, var):
        ...
        pass
        # ### START CODE HERE ###
        def arc_consistency_function(var1, var2):
            list_checker = []
            for val2 in self.domains[var2]:
                ...
                if not arc_consistency:
                    list_checker.append(val2)

            if len(list_checker) > 0:
                for piece in list_checker:
                    self.domains[var2].remove(piece)
                return True
            else:
                return False
        var_set = set([var])
        while len(var_set) > 0:
            random_var = var_set.pop()
            for var2 in self.csp.get_neighbor_vars(random_var):
                domain_change = arc_consistency_function(random_var, var2)
                if domain_change:
                    var_set.add(var2)
        # ### END CODE HERE ###
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    pass
    # ### START CODE HERE ###
    result = ('sum', name, 'iteration tracker')
    if len(variables) > 0:
    ...
    for i, X_i in enumerate(variables):
        partial_result = ('sum', name, i)
        if i > 0:
            pair_list1 = []
            for a in range(maxSum + 1):
                for b in range(maxSum + 1):
                    pair_list1.append((a, b))
            csp.add_variable(partial_result, pair_list1) #add variable as always
            csp.add_binary_factor(('sum', name, i - 1), partial_result, lambda x, y: x[1] == y[0]) #check consistency with past partial_result and partial_result because i > 0 so you can do it
        else:
            pair_list2 = []
            for b in range(maxSum + 1):
                pair_list2.append((0, b))
            csp.add_variable(partial_result, pair_list2) #add variable as always
            csp.add_unary_factor(partial_result, lambda x: x[0] == 0) #check consistency as unary factor because i = 0
        csp.add_binary_factor(partial_result, X_i, lambda x, y: (x[0] + y) == x[1]) #check consistency with partial_result and X_i from variables given independent of the iteration
    csp.add_binary_factor(partial_result, result, lambda x, y: x[1] == y) #check consistency with partial_result and the result from beginning after for loop is complete
        ...
        for quarter in self.profile.quarters:
            ...
            for request in self.profile.requests:
                for cid in request.cids:
                    ...
                    min = self.bulletin.courses[cid].minUnits
                    max = self.bulletin.courses[cid].maxUnits
                    csp.add_variable(variable, list(range(min, max + 1)) + [0])
                    csp.add_binary_factor((request, quarter), variable, lambda cid_of_request, unit: unit > 0 if cid == cid_of_request else unit == 0)
            sum_for_quarter = get_sum_variable(csp, quarter, variables, self.profile.maxUnits)
            csp.add_unary_factor(sum_for_quarter,lambda a: self.profile.maxUnits >= a >= self.profile.minUnits)
        # ### END CODE HERE ###
