...
def learnPredictor(trainExamples, testExamples, featureExtractor, numIters, eta):
    ...
    for i in range(len(trainExamples)):
        # x, y = trainExamples[i]
        features = featureExtractor(trainExamples[i][0])
    ...
    def predictor(x):
        pred = (dotProduct(weights, featureExtractor(x)))
        if pred > 0:
            return 1
        else:
            return -1

    for t in range(numIters):  # maybe multipy by some value for more iterations
        for i in range(len(trainExamples)):
            features = featureExtractor(trainExamples[i][0])
            y = trainExamples[i][1]
...
def generateDataset(numExamples, weights):
    ...
    def generateExample():
        ...
        y = weights[k]
        phi[k] = random.random()
        #y = weights.dot(phi) + random.random
        # ### END CODE HERE ###
        return (phi, y)
