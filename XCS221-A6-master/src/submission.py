    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.numRows):
            for column in range(self.belief.numCols):
                distance_from_agent = math.sqrt(((util.rowToY(row) - agentY) ** 2) + ((util.colToX(column) - agentX) ** 2))
                probability = util.pdf(distance_from_agent, Const.SONAR_STD, observedDist)
